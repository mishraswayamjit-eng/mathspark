...
export function SkeletonGrid({ count = 16 }: { count?: number }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      {[...Array(count)].map((_, i) => (
        <div key={i} className="bg-white rounded-2xl p-4 border-2 border-gray-100 flex flex-col gap-2 h-36 animate-pulse">
          <div className="h-3 bg-gray-200 rounded w-1/3" />
          <div className="h-4 bg-gray-200 rounded w-3/4" />
          <div className="h-5 bg-gray-200 rounded-full w-1/2" />
          <div className="mt-auto h-1.5 bg-gray-200 rounded-full w-full" />
        </div>
      ))}
    </div>
  );
