...
// PATCH /api/student — update mutable student fields (parentEmail, parentWhatsApp, name)
export async function PATCH(req: Request) {
  const body = await req.json() as { studentId?: string; parentEmail?: string; parentWhatsApp?: string; name?: string };
  const { studentId, parentEmail, parentWhatsApp, name } = body;
