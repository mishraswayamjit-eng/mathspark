...
export async function POST(req: Request) {
  ...
  const { studentId, questionId, topicId, selected, isCorrect, hintUsed = 0, timeTakenMs = 0, misconceptionType = null } = body;
  ...
  const attempt = await prisma.attempt.create({
    data: { studentId, questionId, selected, isCorrect, hintUsed, timeTakenMs, misconceptionType },
  });
